...
class TestHierarchicalReward:
    ...
    def test_single_good_chain_no_longer_overinflates(self):
        """One lucky chain should not dominate reward under mean TED/F1."""
        weak = [EMPTY_CHAIN, EMPTY_CHAIN, EMPTY_CHAIN]
        mix = [SHORT_CHAIN] + weak
        all_good = [SHORT_CHAIN] * 4
        r_mix = compute_reward(**self._base_kwargs(
            predicted_chains=mix,
            true_chain=SHORT_CHAIN,
        ))
        r_all = compute_reward(**self._base_kwargs(
            predicted_chains=all_good,
            true_chain=SHORT_CHAIN,
        ))
        assert r_all.total > r_mix.total

    def test_quality_gated_entropy_bonus(self):
        """High diversity with poor TED should not earn entropy bonus."""
        noisy = [
        ...
        res = compute_reward(**self._base_kwargs(
            predicted_chains=noisy,
            true_chain=[PrimitiveType.QUOTE_FABRICATE, PrimitiveType.ENTITY_SUBSTITUTE],
        ))
        assert res.chain_entropy_bonus == pytest.approx(0.0)
