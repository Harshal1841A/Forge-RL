...
class RedAgent:
    ...
    def commit_action(self, action: RedAction):
        """
        Explicitly commit an accepted action to the chain.
        Called by environment after action is applied to ClaimGraph.
        """
        if action.primitive not in self._current_chain or \
                len(self._current_chain) < K_MAX:
