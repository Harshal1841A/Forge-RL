...
PRD v9.0 Section 3.1 specification:
  ...
  set_overlap_score: Jaccard similarity between predicted set and
    ...
    Missing-coverage penalty: -0.08 per true primitive not predicted.
...
Examples:
  ...
  predict=[A,B],   true=[A,B,C] → lower than prior formulation (coverage penalty)
...
def tactic_edit_distance(predicted: list, true_chain: list) -> float:
    ...
    # Coverage penalty: punish under-predicting true primitives.
    # Without this, short conservative chains can score artificially high.
    missing = len(true_set - pred_set)
    missing_penalty = min(0.40, missing * 0.08)  # cap keeps score bounded

    set_overlap_score = max(0.0, jaccard - hallucination_penalty - missing_penalty)
